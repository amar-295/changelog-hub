...
const createRelease = asyncHandler(async (req, res) => {
  const { title, version, content, category, status } = req.body;
  ...
  if (category && !validateCategories.includes(category)) {
    throw new ApiError(
      400,
      `Category must be one of ${validateCategories.join(', ')}`
    );
  }
  ...
  if (status && !validateStatus.includes(status)) {
    throw new ApiError(
      400,
      `Status must be one of ${validateStatus.join(', ')}`
    );
  }
  ...
  try {
    ...
    if (release.status === 'published') {
      ...
      if (subscribers.length > 0) {
        sendReleaseNotification(subscribers, release, workspace);
      }
    }

    return res
      .status(201)
      .json(new ApiResponse(201, release, 'Release created successfully'));
  } catch (error) {
    if (error.code === 11000) {
...
const getReleaseById = asyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, release, 'Release fetched successfully'));
});
...
const deleteRelease = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .json(new ApiResponse(200, {}, 'Release deleted successfully'));
  } catch (error) {
    ...
    return res
      ...
      .json(
        new ApiResponse(
          200,
          unpublishedRelease,
          'Release unpublished successfully'
        )
      );
  } catch (error) {
